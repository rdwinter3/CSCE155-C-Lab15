...
SQLRETURN addVideoGame(char* name, int publisherID, SQLHANDLE handle)
{
    ...
    if(g != NULL) {
      printf("Attempt to add game %s aborted: game with that name already exists.\n", name);
      return SQL_NO_DATA;
    }
    ...
    asprintf(&command, "INSERT INTO game(name, publisher_id) VALUES (?, ?)", name, publisherID);
...
SQLRETURN addPlatform(char* name, SQLHANDLE handle)
{
  ...
  if(p != NULL) {
    printf("Attempt to add platform %s aborted: platform with that name already exists.\n", name);
    return SQL_NO_DATA;
  }
...
SQLRETURN addPublisher(char* name, SQLHANDLE handle)
{
    ...
    if(p != NULL) {
      printf("Attempt to add publisher %s aborted: publisher with that name already exists.\n", name);
      return SQL_NO_DATA;
    }
...
SQLRETURN addAvailability(int gameID, int platformID, int publishYear, SQLHANDLE handle)
{
    //Calculates needed string length for the command
    SQLRETURN returnVal;
    SQLCHAR* command;
    ...
    //Send game info to DB
    asprintf(&command, "INSERT INTO availability(game_id, platform_id,"
             "publish_year) VALUES(%d, %d, %d)", gameID, platformID, publishYear);
...
void getAvailability(VideoGame* game, SQLHANDLE handle)
{
    ...
    //Store number of entries returned by the SQL command and allocate space for platforms
    ...
    //Fills all platforms in game with year and ID (need separate select for platform name)
    for (i = 0; SQL_SUCCEEDED(returnVal = SQLFetch(handle)); i++) 
    {
    ...
    for (i = 0; i < numSelRows; i++)
    {
        ...
        asprintf(&command, "SELECT * FROM platform WHERE platform_id = %d", platforms[i]->id);
        returnVal = SQLExecDirect(handle, command, SQL_NTS);
        
        //Success?
        if (returnVal == SQL_SUCCESS || returnVal == SQL_SUCCESS_WITH_INFO)
        {
            SQLBindCol(handle, 2, SQL_C_CHAR, platforms[i]->name, sizeof(char) * 1024, NULL);
            SQLFetch(handle);
        }
        else
        {
...
VideoGame* getGame(char* name, SQLHANDLE handle)
{
	...
	if(numResults == 0) {
	  ...
	  printf("WARNING: multiple records (%d) exist for game '%s', returning the first one...\n", numResults, name);
	}  

        //Maps expected columns to structure/publisherID
        SQLBindCol(handle, 1, SQL_INTEGER, &thisGame->id , sizeof(thisGame->id), NULL);
        SQLBindCol(handle, 2, SQL_C_CHAR, &thisGame->name, sizeof(char) * 1024, NULL);
    ...
    //Publisher's default value should be NULL for each game (incase no publisher is found)
    thisGame->publisher = NULL;

    //Get publisher name from DB given the ID found in the previous query
    char pubName[1024];
    asprintf(&command, "SELECT * FROM publisher WHERE publisher_id = %d", publisherID);
    returnVal = SQLExecDirect(handle, command, SQL_NTS);
...
Platform* getPlatform(char* name, SQLHANDLE handle)
{
    ...
    if(returnVal == SQL_SUCCESS || returnVal == SQL_SUCCESS_WITH_INFO) 
    {
	  ...
	  return NULL;
  	  ...
  	  printf("WARNING: multiple records (%d) exist for platform '%s', returning the first one...\n", numResults, name);
        }

        //Maps expected columns to structure/publisherID
        SQLBindCol(handle, 1, SQL_INTEGER, &thisPlatform->id , sizeof(thisPlatform->id), NULL);
        SQLBindCol(handle, 2, SQL_C_CHAR, &thisPlatform->name, sizeof(char) * 1024, NULL);
...
Publisher* getPublisher(char* name, SQLHANDLE handle)
{
    ...
    if(returnVal == SQL_SUCCESS || returnVal == SQL_SUCCESS_WITH_INFO) 
    {
	  ...
	  printf("WARNING: multiple records (%d) exist for publisher '%s', returning the first one...\n", numResults, name);
        }

        //Maps expected columns to structure/publisherID
        SQLBindCol(handle, 1, SQL_INTEGER, &thisPublisher->id , sizeof(thisPublisher->id), NULL);
        SQLBindCol(handle, 2, SQL_C_CHAR, &thisPublisher->name, sizeof(char) * 1024, NULL);
